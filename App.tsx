...
const App: React.FC = () => {
  ...
  const clearChat = () => {
    if (confirm(lang === 'pt' ? 'Limpar todo o histórico de conversa?' : 'Clear all chat history?')) {
      const welcomeMsg = { id: 'welcome', role: Role.ASSISTANT, content: t.welcome, timestamp: new Date() };
            ...
            >
              <div className="p-6 md:p-8 border-b border-white/5 flex justify-between items-center bg-white/2">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-xl font-bold text-white tracking-tight">Console de Implementação Vasstos</h2>
                    <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Setup para www.vasstos.com</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <button 
                    onClick={clearChat}
                    className="flex items-center gap-2 px-4 py-2 hover:bg-red-500/10 text-red-500 rounded-xl transition-colors text-xs font-bold"
                  >
                    <Trash2 size={14} />
                    Limpar Chat
                  </button>
              ...
              <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-8 scrollbar-hide">
                <div className="flex flex-wrap gap-2 p-1 bg-black/40 rounded-2xl border border-white/5 w-fit">
                  <button onClick={() => setActiveTab('github')} className={cn("flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all", activeTab === 'github' ? "bg-blue-600 text-white" : "text-slate-500 hover:text-slate-300")}>
                    <Github size={14} /> 1. Hospedar (GitHub)
                  </button>
                  <button onClick={() => setActiveTab('googlesites')} className={cn("flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all", activeTab === 'googlesites' ? "bg-blue-600 text-white" : "text-slate-500 hover:text-slate-300")}>
                    <Layout size={14} /> 2. Integrar (Sites)
                  </button>
                  <button onClick={() => setActiveTab('standard')} className={cn("flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all", activeTab === 'standard' ? "bg-blue-600 text-white" : "text-slate-500 hover:text-slate-300")}>
                    <Globe size={14} /> HTML Externo
                  </button>
                </div>
                ...
                {activeTab === 'github' && (
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in duration-500">
                    <div className="space-y-6">
                      <div className="p-6 bg-green-500/10 border border-green-500/20 rounded-3xl relative overflow-hidden">
                        <div className="absolute top-0 right-0 p-4 opacity-10">
                          <Sparkles size={80} />
                        </div>
                        <div className="flex items-center gap-3 mb-4">
                          <CircleCheck size={20} className="text-green-500" />
                          <h4 className="text-sm font-bold text-white uppercase tracking-wider">Hospedagem Confirmada?</h4>
                        </div>
                        <p className="text-xs text-slate-400 mb-6 relative z-10">
                          Se você já fez o upload para o GitHub Pages, seu script está vivo! Agora vamos gerar o código de integração.
                        </p>
                        
                        <button 
                          onClick={() => setActiveTab('googlesites')}
                          className="w-full flex items-center justify-center gap-2 py-4 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-2xl transition-all group shadow-lg shadow-blue-900/20"
                        >
                          Próximo: Gerar Código
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2 block">Seu Usuário</label>
                          <input type="text" value={ghUser} onChange={(e) => setGhUser(e.target.value)} className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-xs text-white focus:border-blue-500 outline-none" placeholder="ex: vasstos" />
                        </div>
                        <div>
                          <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2 block">Repositório</label>
                          <input type="text" value={ghRepo} onChange={(e) => setGhRepo(e.target.value)} className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-xs text-white focus:border-blue-500 outline-none" placeholder="ex: chatbot" />
                        </div>
                      </div>
                    </div>

                    <div className="space-y-6">
                      <div className="p-6 bg-white/5 border border-white/5 rounded-3xl group">
                        <h4 className="text-xs font-bold text-white uppercase tracking-wider mb-2 flex justify-between">
                          URL do Script
                          <span className="text-green-500 text-[10px] animate-pulse">Online</span>
                        </h4>
                        <code className="text-[10px] text-blue-400 block break-all bg-black/40 p-4 rounded-xl border border-white/5 font-mono group-hover:border-blue-500/30 transition-colors">
                          {scriptUrl}
                        </code>
                        <div className="mt-4 flex items-start gap-3 p-3 bg-blue-600/5 rounded-xl border border-blue-500/10">
                          <Info size={14} className="text-blue-500 mt-0.5 shrink-0" />
                          <p className="text-[10px] text-slate-500 leading-relaxed font-medium italic">
                            Esta URL aponta para o arquivo compilado que o Google Sites irá ler. Certifique-se de que o arquivo se chama exatamente <code>index.js</code> no seu GitHub.
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-6">
                      <div className="relative group">
                        <div className="flex justify-between items-center mb-2">
                          <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Snippet para o Google Sites</label>
                          <button onClick={() => copyToClipboard(googleSitesSnippet)} className="text-[10px] font-bold px-3 py-1 bg-blue-600 hover:bg-blue-500 text-white rounded-lg transition-all flex items-center gap-1.5 shadow-lg shadow-blue-900/40">
                            {copied ? <Check size={12} /> : <Copy size={12} />} {copied ? 'Copiado!' : 'Copiar para o Sites'}
                          </button>
                        </div>
                        <div className="bg-black/60 p-5 rounded-2xl border border-white/5 font-mono text-[11px] leading-relaxed text-blue-300 overflow-x-auto min-h-[200px] relative">
                          <pre>{googleSitesSnippet}</pre>
                          {copied && (
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 bg-blue-600/10 backdrop-blur-[2px] flex items-center justify-center rounded-2xl border border-blue-500/50">
                               <div className="flex items-center gap-2 bg-slate-900 px-4 py-2 rounded-full border border-blue-500 text-blue-400 font-bold text-xs">
                                 <CircleCheck size={14} /> Pronto para colar no Sites!
                               </div>
                            </motion.div>
                          )}
                        </div>
                    ...
                    <div className="space-y-6">
                      <div className="p-6 bg-slate-800/40 border border-white/5 rounded-3xl">
                        <h4 className="text-xs font-bold text-white uppercase tracking-wider mb-4 flex items-center gap-2">
                          <Layout size={16} className="text-blue-500" /> Checklist Final:
                        </h4>
                        <ol className="space-y-4 text-[11px] text-slate-400">
                          <li className="flex gap-3">
                            <span className="bg-blue-600/20 text-blue-400 w-5 h-5 flex items-center justify-center rounded-md font-bold shrink-0">1</span> 
                            No Google Sites: <strong>Inserir > Incorporar > Incorporar código</strong>.
                          </li>
                          <li className="flex gap-3">
                            <span className="bg-blue-600/20 text-blue-400 w-5 h-5 flex items-center justify-center rounded-md font-bold shrink-0">2</span> 
                            Cole o código acima e clique em <strong>Próximo > Inserir</strong>.
                          </li>
                          <li className="flex gap-3">
                            <span className="bg-blue-600/20 text-blue-400 w-5 h-5 flex items-center justify-center rounded-md font-bold shrink-0">3</span> 
                            <strong>Dica de UX:</strong> Aumente o tamanho da caixa de incorporação para ocupar o rodapé direito.
                          </li>
                        </ol>
                      </div>
                      <div className="p-4 border border-yellow-500/20 bg-yellow-500/5 rounded-2xl">
                         <p className="text-[10px] text-yellow-500/80 font-medium">
                           ⚠️ Se o bot não carregar, verifique se a URL no GitHub está correta e se o repositório é <strong>Público</strong>.
                         </p>
                      </div>
                    </div>
                  </div>
                ...
                {activeTab === 'standard' && (
                  <div className="p-10 bg-white/5 border border-white/5 rounded-[2.5rem] text-center max-w-2xl mx-auto">
                    <Globe size={48} className="mx-auto mb-6 text-blue-500 opacity-50" />
                    <h3 className="text-xl font-bold text-white mb-2">Uso em WordPress ou Outros</h3>
                    <p className="text-sm text-slate-500 mb-8 leading-relaxed">
                      Se você decidir migrar da plataforma Google Sites para um servidor próprio ou WordPress, use este snippet simplificado.
                    </p>
                    <div className="bg-black/60 p-6 rounded-2xl border border-white/5 text-left mb-8 overflow-x-auto relative group">
                       <button onClick={() => copyToClipboard(standardSnippet)} className="absolute top-4 right-4 p-2 bg-white/10 hover:bg-white/20 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity">
                         <Copy size={16} className="text-white" />
                       </button>
                      <pre className="text-blue-300 text-xs font-mono">{standardSnippet}</pre>
                    </div>
                    <div className="flex justify-center gap-4">
                       <div className="flex items-center gap-2 text-xs text-slate-500">
                         <CircleCheck size={14} className="text-green-500" /> Scripts assíncronos
                       </div>
                       <div className="flex items-center gap-2 text-xs text-slate-500">
                         <CircleCheck size={14} className="text-green-500" /> Baixa latência
                       </div>
                    </div>
                  </div>
                )}
        ...
        {isOpen && (
          ...
          >
            <header className="px-6 py-5 flex items-center justify-between border-b border-white/5 bg-gradient-to-b from-white/5 to-transparent shrink-0">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => setShowInstallGuide(true)} className="p-2 hover:bg-white/5 rounded-xl transition-colors text-slate-400 hover:text-white" title="Configurações de Deploy">
                  <Settings size={16} />
                </button>
                <button onClick={toggleLanguage} className="flex items-center gap-1.5 text-[10px] font-bold px-2.5 py-1.5 rounded-xl bg-white/5 border border-white/5 hover:border-blue-500/50 transition-all text-slate-300">
            ...
            <AnimatePresence>
              {showScrollButton && (
                <motion.button initial={{ opacity: 0, scale: 0.8, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.8, y: 10 }} onClick={() => scrollToBottom()} className="absolute bottom-[110px] right-6 p-3 bg-blue-600 text-white rounded-full shadow-2xl border border-white/10 hover:bg-blue-500 transition-colors z-20 flex items-center justify-center">
                  <ArrowDown size={18} />
                </motion.button>
              )}
            </AnimatePresence>
